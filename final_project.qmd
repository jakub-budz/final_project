...
```{python}
file_path = '/Users/laratamer/Desktop/Crimes_-_2001_to_Present_20241127.csv'
...
```{python}
community_areas_path = '/Users/laratamer/Desktop/final_project/chicago-community-areas.csv'
community_areas = pd.read_csv(community_areas_path, header = None)
...
community_areas.rename(columns = {'Community Area': 'Community Area Number', 'name': 'Community Area Name'}, inplace = True)
...
crimes_by_area = crimes_data.groupby(['Community Area', 'Community Area Name', 'Primary Type']).size().reset_index(name = 'Count')

# Sort by 'Community Area', 'Count' in descending order
crimes_by_area_sorted = crimes_by_area.sort_values(['Community Area', 'Count'], ascending = [True, False])
...
y_positions = np.arange(len(most_common_crimes)) * 1.5  # Add extra spacing by multiplying positions
...
plt.legend(handles, crime_types, title = 'Primary Type', bbox_to_anchor = (1.05, 1), loc = 'upper left', fontsize = 10)
...
plt.figure(figsize = (8, 6))
plt.bar(crime_counts_sorted['Primary Type'], crime_counts_sorted['Community Areas Count'], color = 'grey', edgecolor = 'black')
...
# Ensure the 'Date' column is in datetime format
most_common_crimes_with_timing['Date'] = pd.to_datetime(most_common_crimes_with_timing['Date'], errors = 'coerce')
...
crimes_data['Day'] = crimes_data['Date'].dt.day_name().str[:2]  # Extract abbreviations (Mo, Tu, etc.)
...
```{python}
file2_path = '/Users/laratamer/Downloads/Violence_Reduction_-_Victims_of_Homicides_and_Non-Fatal_Shootings_20241125.csv'

violence_reduction_data = pd.read_csv(file2_path)
...
chart_all = alt.Chart(summary_all).mark_bar().encode(
    ...
    width = 300,
    height = 400,
    title = "Homicides and Non-Fatal Shootings Across Priority Community Areas (2018–2020 vs. 2021–2023)"
)
...
# Filter data for non-priority community areas (no filtering for specific VICTIMIZATION_PRIMARY values)
...
# Remove rows outside the desired periods
data_filtered_non_priority_all = data_filtered_non_priority_all[data_filtered_non_priority_all['Period'].notna()]

# Group and count incidents by community area and period
summary_non_priority_all = data_filtered_non_priority_all.groupby(['COMMUNITY_AREA', 'Period']).size().reset_index(name = 'Count')

# Sort community areas alphabetically for consistency
summary_non_priority_all['COMMUNITY_AREA'] = summary_non_priority_all['COMMUNITY_AREA'].astype(str)
...
chart_non_priority_all = alt.Chart(summary_non_priority_all).mark_bar().encode(
    ...
    height = 500,
    title = "Homicides and Non-Fatal Shootings Across Non-Priority Community Areas (2018–2020 vs. 2021–2023)"
)
